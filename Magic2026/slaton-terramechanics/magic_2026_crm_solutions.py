...
marked GIVEN. Every EXERCISE is about the CRM/FSI part: the soil, coupling the
plate to it, driving it, and stepping the coupled problem.

THIS IS THE SOLUTION FILE
Every EXERCISE is completed. The exercise file (magic_2026_crm_demo.py) hands
off to this one when run with --solutions.

TWO FSI GOTCHAS BAKED IN (both matter):
...
pychrono fsi (and vsg3d for the window) modules. Then:
    python magic_2026_crm_solutions.py
At the end a reaction-pressure-vs-sinkage curve is written to
bevameter_pressure_sinkage.csv (and plotted if matplotlib is installed).
...
# EXERCISE 1 - soil properties (SOLUTION)
# The CRM soil's SPH physics, using the mu(I) rheology.
mat_props = fsi.ElasticMaterialProperties()
mat_props.density = density
mat_props.Young_modulus = youngs_modulus
mat_props.Poisson_ratio = poisson_ratio
_rheo = getattr(fsi, "RheologyCRM_MU_OF_I", None)
if _rheo is not None:
    mat_props.rheology_model = _rheo
mat_props.mu_I0 = 0.04
mat_props.mu_fric_s = mu_s
mat_props.mu_fric_2 = mu_2
mat_props.average_diam = 0.0002
if hasattr(mat_props, "cohesion_coeff"):
    mat_props.cohesion_coeff = cohesion
elif hasattr(mat_props, "cohesion"):
    mat_props.cohesion = cohesion
...
# EXERCISE 2 - create the soil bed (SOLUTION)
# Turn each sampled point into an SPH particle, initialized with the geostatic
# (overburden) pressure at its depth so the bed starts in equilibrium.
# AddSPHParticle args:
...
for p in points:
    pre = rho0 * gz * (bed_depth - p.z)   # overburden pressure at this depth
...
# EXERCISE 3 - couple the plate to the SPH soil (SOLUTION)
# Fill the plate cylinder with boundary (BCE) markers and register it as an FSI
# body so the soil can push back on it. (On current Chrono main this second call
# is sys_fsi.AddRigidBody.)
...
# EXERCISE 4 - drive the plate (SOLUTION)
# A linear position motor between the plate and the fixed container drives the
# plate straight down at a constant rate (position = -velocity * t).
...
while sinkage <= target_sinkage:
    ...
    # EXERCISE 5 (capstone) - advance the coupled SPH + multibody problem (SOLUTION)
    # One call steps the fluid and the plate/motor together by dt.
